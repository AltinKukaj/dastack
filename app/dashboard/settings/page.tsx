...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    authClient
      ...
      .catch(() =>
        setSessionMsg({
          type: "err",
          text: "Could not load active sessions.",
        }),
      )
      ...
      .catch((err: unknown) => {
        const message =
          err instanceof Error
            ? err.message
            : "Could not load passkeys for this account.";
        ...
        if (
          ...
          setPasskeyMsg({
            type: "err",
            text: "Passkey table is missing in the current database. Run `bun db:push` or set DISABLE_PASSKEY=true.",
          });
        } else {
          setPasskeyMsg({
            type: "err",
            text: "Could not load passkeys for this account.",
          });
        }
      })
  ...
  const handleRevokeSession = async (s: SessionInfo) => {
    ...
    try {
      ...
      setSessionMsg({
        type: "err",
        text: "Could not revoke that session.",
      });
    } finally {
      setRevokingId(null);
    }
  ...
  const handleRevokeAllOther = async () => {
    ...
    try {
      ...
      setSessionMsg({
        type: "ok",
        text: "All other sessions were revoked.",
      });
    } catch {
      setSessionMsg({
        type: "err",
        text: "Could not revoke other sessions.",
      });
    } finally {
      setRevokingId(null);
    }
                ...
                {passkeysLoading ? (
                  ...
                  passkeys.map((passkey) => (
                    ...
                    >
                      ...
                      >
                        {passkeyActionLoading === passkey.id ? "..." : "Delete"}
                      </button>
                    </div>
                  ))
                )}
