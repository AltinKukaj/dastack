...
export function SignUpForm({ callbackURL }: SignUpFormProps) {
  ...
  useEffect(() => {
    getClientFeatureFlags()
      .then((data) => setConfig({ emailEnabled: data.email, providers: data.providers }))
      .catch(() => setConfig({ emailEnabled: false, providers: { discord: false, google: false, github: false } }));
  }, []);
      ...
      <div className="flex flex-col items-center gap-4 py-6 text-center">
        ...
        <div>
          ...
          <p className="mt-1.5 text-sm text-neutral-400">
            We sent a verification link to <span className="text-neutral-200">{email}</span>. Click it to activate your account.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3">
        <div>
          <label htmlFor="signup-name" className="mb-1.5 block text-xs text-neutral-600">Full name</label>
          <input id="signup-name" type="text" autoComplete="name" placeholder="Jane Doe" value={name} onChange={(e) => setName(e.target.value)}
        ...
        <div>
          <label htmlFor="signup-username" className="mb-1.5 block text-xs text-neutral-600">Username <span className="text-neutral-700">(optional)</span></label>
          <input id="signup-username" type="text" autoComplete="username" placeholder="janedoe" value={username} onChange={(e) => setUsername(e.target.value)}
            className="w-full rounded-lg border border-neutral-800 bg-transparent px-4 py-2.5 text-sm text-white placeholder:text-neutral-700 outline-none transition focus:border-neutral-600" />
        </div>

        <div>
          <label htmlFor="signup-email" className="mb-1.5 block text-xs text-neutral-600">Email</label>
          <input id="signup-email" type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)}
            className="w-full rounded-lg border border-neutral-800 bg-transparent px-4 py-2.5 text-sm text-white placeholder:text-neutral-700 outline-none transition focus:border-neutral-600" />
        </div>

        <div>
          <label htmlFor="signup-password" className="mb-1.5 block text-xs text-neutral-600">Password</label>
          <input id="signup-password" type="password" autoComplete="new-password" placeholder="At least 8 characters" value={password} onChange={(e) => setPassword(e.target.value)}
            className="w-full rounded-lg border border-neutral-800 bg-transparent px-4 py-2.5 text-sm text-white placeholder:text-neutral-700 outline-none transition focus:border-neutral-600" />
        </div>

        <div>
          <label htmlFor="signup-confirm-password" className="mb-1.5 block text-xs text-neutral-600">Confirm password</label>
          <input id="signup-confirm-password" type="password" autoComplete="new-password" placeholder="Repeat your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
        ...
        <button
          type="button"
          disabled={loading === "email" || !name.trim() || !email.trim() || !password || !confirmPassword}
        ...
        >
          {loading === "email" ? <span className="flex items-center justify-center gap-2"><Spinner dark /> Creating account...</span> : "Create account"}
        </button>

        <p className="text-xs text-neutral-600">Passwords are checked against known breaches via <span className="text-neutral-500">HaveIBeenPwned</span>.</p>
      </div>
